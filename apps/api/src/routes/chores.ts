...
choresRouter.post(
  "/chores",
  zValidator("json", createChoreSchema),
  async (c) => {
    const userId = getUserId(c);
    if (!userId) return c.json({ error: "Unauthorized" }, 401);
...
choresRouter.patch(
  "/chores/:id",
  zValidator("json", updateChoreSchema),
  async (c) => {
    const userId = getUserId(c);
    if (!userId) return c.json({ error: "Unauthorized" }, 401);
...
choresRouter.post("/chores/:id/complete", async (c) => {
  ...
  try {
    ...
    if (msg === "Chore not found") return c.json({ error: msg }, 404);
    if (msg.includes("Not allowed") || msg.includes("paused")) {
